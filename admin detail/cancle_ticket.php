...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM pass_details WHERE id = '$delete_id'";
    if (mysqli_query($conn, $delete_query)) {
        echo "<script>alert('Record deleted successfully');</script>";
        // Redirect back to this page to refresh the table
        echo "<script>window.location.href = 'cancle_ticket.php';</script>";
    } else {
        echo "<script>alert('Error deleting record: " . mysqli_error($conn) . "');</script>";
    }
}

$query = "SELECT * FROM pass_details WHERE status = 'cancelled'"; 
...
<table class="user-detail2" id="adminTable"> 
	<tr> 
		<th colspan="12"><h2>All canceled Ticket Booking</h2></th> <!-- Changed colspan to 3 -->
	</tr> 
	...
	<?php while($rows=mysqli_fetch_assoc($result)) { ?> 
		<td><?php echo $rows['id']; ?></td> 
            ...
            <td><button style="background:red; height:50px;width:100px;border-radius:10px;"><a href="?delete_id=<?php echo $rows['id']; ?>" style="text-decoration:none; color:white;">Delete</a></button></td>
