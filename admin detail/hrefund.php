...
if (isset($_GET['refund_id'])) {
    ...
    // Get the total_pay value from the database
    $query = "SELECT total_pay FROM hotel_cust_detail WHERE uid = '$refund_id'";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);
    $total_pay = $row['total_pay'];

    // Update the refund column with the total_pay value
    $update_query = "UPDATE hotel_cust_detail SET refund = '$total_pay' WHERE uid = '$refund_id'";
    if (mysqli_query($conn, $update_query)) {
        echo "<script>alert('Refund processed successfully!');</script>";
		//Redirect back to this page to refresh the table
        echo "<script>window.location.href = 'hrefund.php';</script>";
    } else {
        echo "<script>alert('Error occurred while refunding.');</script>";
    }
}

$query = "SELECT * FROM hotel_cust_detail WHERE status = 'cancelled' AND refund = '0'";
$result = mysqli_query($conn, $query);
?>
...
<table class="user-detail2" id="adminTable"> 
	...
	<?php while($rows=mysqli_fetch_assoc($result)) { ?> 
		<tr> 
			...
			<td><?php echo $rows['total_pay']; ?></td> 
            <td><button style="background:Green; height:50px;width:100px;border-radius:10px;">
			<a href="?refund_id=<?php echo $rows['uid']; ?>" style="text-decoration:none; color:white;"><i class="bi bi-cash-coin"></i>Refund</a>
		     </button></td>
